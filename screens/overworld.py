...
class Overworld(object):
    ...
    def _show_pause_menu(self):
        ...
        menu_content = screens.menu.content.PauseMenu(self)
        menu_title = False
        menu_animation = False
        scr_data = pygame.image.tostring(self.engine.screen, 'RGBA')    # maak een screen capture
        menu_scr_capt = pygame.image.frombuffer(scr_data, self.engine.screen.get_size(), 'RGBA').convert()
        menu_select = 0
        menu_object = screens.menu.display.Display(self.engine.screen, self.engine.audio,
                                                   menu_content,
                                                   menu_title,
                                                   menu_animation,
                                                   menu_scr_capt,
                                                   menu_select)
        self.engine.gamestate.push(menu_object)
