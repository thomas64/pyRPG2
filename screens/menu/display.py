...
class Display(object):
    ...
    def __init__(self, screen, audio, menu_content, title, animation, scr_capt, cur_item=0):
        ...
        if self.scr_capt:
            self.screen.blit(self.scr_capt, (0, 0))             # gooi over het hele scherm de overworld achtergrond
    ...
    def single_input(self, event):
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if event.button == keys.LEFTCLICK:
                for item in self.menu_texts:
                    if item.rect.collidepoint(event.pos):
                        ...
                        self.menu_content.on_select(item.func, self)
            ...
            if event.key in keys.SELECT:
                self.audio.play_sound(self.audio.select)
                self.menu_content.on_select(self.menu_texts[self.cur_item].func, self)
            elif event.key == keys.DELETE:
                self.audio.play_sound(self.audio.select)
                self.menu_content.on_delete(self.menu_texts[self.cur_item].func, self.cur_item)
